    ...
    <div class="toolbar" id="kt_toolbar">
        ...
        <div id="kt_toolbar_container" class="container-fluid d-flex flex-stack">
            ...
            <div data-kt-place="true" data-kt-place-mode="prepend" data-kt-place-parent="{default: '#kt_content_container', 'lg': '#kt_toolbar_container'}" class="page-title d-flex align-items-center me-3 flex-wrap mb-5 mb-lg-0 lh-1">
                <!--begin::Title-->
                <h1 class="d-flex align-items-center text-dark fw-bolder my-1 fs-3">Blog Comment</h1>
                <!--end::Title-->
            </div>
        ...
        <div id="kt_content_container" class="container-fluid">
            ...
            <div class="card mb-10">
                <div class="card-body d-flex align-items-center p-5 p-lg-8">
                    ...
                    <!--begin::Description-->
                    <div class="ms-6">
                        <p class="list-unstyled text-gray-600 fw-bold fs-6 p-0 m-0">Blog Comment Manage</p>
                    </div>
                    <!--end::Description-->
                </div>
            </div>
                    ...
                    <div class="card mb-5 mb-xl-8">
                        ...
                        <div class="card-header border-0 pt-5">
                            <h3 class="card-title align-items-start flex-column">
                                <span class="card-label fw-bolder fs-3 mb-1">All Blog Comment</span>
                                <!-- <span class="text-muted mt-1 fw-bold fs-7">Over 500 orders</span> -->
                            </h3>
                            <div class="card-toolbar">
                                
                                <a href="{{ url('admin/blog/create') }}" class="btn btn-sm btn-primary">Create Blog</a>
                            </div>
                        </div>
                                    ...
                                    @foreach($alldata as $key => $data)
                                        <tr>
                                            <td> {{ ++$key }} </td>
                                             ...
                                             <td> {{ $data->email }} </td>
                                             <td> {{ $data->comment }} </td>
                                             <td><textarea id="{{ $data->id }}" name="mynots" onchange="insertupdate(this)"> {{ $data->reply }} </textarea></td>
                                             <td> 
                                                @if($data->is_published==1) 
                                                <span class="badge badge-success">Comment published</span>
                                                @else
                                            ...
                                            <td class="text-end">
                                               @if($data->is_published==1) 
                                                 ...
                                                 <a href="{{url('admin/blog-comment/unpublished/'.$data->id)}}" class="btn btn-icon btn-bg-light btn-active-color-primary btn-sm me-1">
                                                <i class="fas fa-thumbs-up blue"></i>
                                                </a>
                                                 @else
                                                    <a href="{{url('admin/blog-comment/published/'.$data->id)}}" class="btn btn-icon btn-bg-light btn-active-color-primary btn-sm me-1">
                                                <i class="fas fa-thumbs-down blue"></i>
                                                </a>
                                                 @endif
                                                
                                                <a id="delete" href="{{url('/admin/blog-comment/delete/'.$data->id)}}" class="btn btn-icon btn-bg-light btn-active-color-primary btn-sm">
...
<script>
    function insertupdate(el){
        ...
        var val=el.value;
            ...
            $.ajax({
                 url: "{{  url('admin/update/blogcomments') }}",
                 type:"GET",
                 data: { 
                     "val": val,
                     "id": id,
