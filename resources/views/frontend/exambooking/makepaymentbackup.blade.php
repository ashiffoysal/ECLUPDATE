...
@if(isset($series) && $todayDate <= $entryDate)
    <section class="contact-page-section installments" style="display:">
        <div class="auto-container">
            <div class="row clearfix">
                <div class="offset-2 col-md-10 text-center mt-10 mycheckdesign">
                    <p>
                        <input type="radio" onclick="pay_section(this)" id="fullpayment_id" value="full_payment" name="radio-group" checked>
                        <label for="fullpayment_id">I want to make full Payment </label>
                        
                        <input type="radio" onclick="pay_section(this)" id="Installments_id" value="installments" name="radio-group">
                        <label for="Installments_id">I want to pay in Installments.</label>
                    </p>
                </div>
            </div>
        </div>
...
@endif
                    ...
                    <div class="inner-column">
                    	 <div class="alert alert-success new2 p-4">
								       ...
								       <span class="alert-body">
								         ...
								         <p class="mb-0" style="color:black !important">Please complete your payment to confirm your exam booking.</p>
								       </span>
								    </div>
                        <div class="sec-title-two">
                            <div class="title" style="color: white;">Total Amount £ {{ $mainprice + $mockprice - $data->discount_amount + $data->admin_specialaccess_amount + $data->special_access_initial_fees}}</div>
                        </div>
                        <h6>Exam includes:</h6>
                        <ul>
	                		@if($data->exam_information ==null)
							...
							@endif
							@if($data->mock_test=='mock_test_yes')
	                        <li>Mock Test Amount: £ {{$mockprice ?? 0}}</li>
					        @endif
							<li>Discount Amount: £ {{$data->discount_amount}}</li>
							<li>Special Access Document Checking Fee: £ {{ $data->special_access_initial_fees }}</li>
                            <li>Special Access Amount: £ {{$data->admin_specialaccess_amount}}</li>
                            
                        </ul>
                        @if($data->main_exam_type=="A Level" || $data->main_exam_type=="AS Level" || $data->main_exam_type=="GCSE" || $data->main_exam_type=="IGCSE")
                        <div class="row mt-5" style="color:#3ec7b0"> 
									<div class="col-md-4">
										...
										<span id="wrong_coupon" style="color:red;display:none">Coupon Code is Worng</span>
										<span id="success_coupon" style="color:green;display:none">Coupon add success</span>
										<span id="warning_coupon" style="color:red;display:none">Already used this Coupon code</span>
									</div>
									<div class="col-md-2">
                             ...
                             <div class="form-group" id="bank_new_payment" style="display:none;">
	                            <form method="post" action="{{ url('bank/payment') }}" enctype="multipart/form-data">
										...
										<div class="row clearfix">
											...
											<label style="font-size:18px">ECL Account Details:</label>
											<span style="font-size: 19px;color:#000;">Account Name: EDU SERVICE LIMITED <br>Account Number: 18901601 <br> Sort Code: 04-06-05</span>
											</div>
											<div class="form-group col-lg-12 col-md-12 col-sm-12">
												<label class="margin" style="font-size: 18px;
    color: #000;">Transaction Details</label>
											</div>
											<!-- Form Group -->
											<div class="form-group col-lg-12 col-md-12 col-sm-12">
												<level style="font-size: 18px;
    color: #000;">Reference </level>
										...
										<input type="hidden" name="booking_id" value="{{ $data->booking_id }}">
											...
											<!-- Form Group -->
											<div class="form-group col-lg-12 col-md-12 col-sm-12">
												<level style="font-size:12px;color:#000 !important">Transaction Screenshot</level><br>
												<input type="file" name="file" style="margin-top: 10px;" accept=".png, .jpg, .jpeg">
												<div class="form-text">Allowed file types: png, jpg, jpeg.</div>
											</div>
											<!-- Form Group -->
											<input type="hidden" name="amount" value="{{ $mainprice + $mockprice + $data->admin_specialaccess_amount - $data->discount_amount + $data->special_access_initial_fees }}">
						                    <input type="hidden" name="booking_id" value="{{ $data->booking_id }}">
							...
							<form action="{{ route('myonlinepayment.session') }}" method="POST">
	    						<input type="hidden" name="_token" value="{{csrf_token()}}">
								  <input type="hidden" name="amount" id="main_amount" value="{{ $mainprice + $mockprice - $data->discount_amount + $data->admin_specialaccess_amount + $data->special_access_initial_fees }}">
								  ...
								  <button class="theme-btn btn-style-five" type="submit" id="checkout-live-button">Click here to pay with card ( £ {{ $mainprice + $mockprice - $data->discount_amount + $data->admin_specialaccess_amount + $data->special_access_initial_fees}} )</button>
							</form>
    ...
    @if($data->main_exam_type=='GCSE' || $data->main_exam_type=='A Level' || $data->main_exam_type=='AS Level' ||  $data->main_exam_type=='IGCSE' )
    <section class="contact-page-section" id="Installments" style="display:none">
                <div class="auto-container">
                        <div class="row clearfix">
                            <div class="col-md-12 mt-10">
                                  <div class="alert alert-success new2 p-4">
								       ...
								       <span class="alert-body">
								         ...
								         <p class="mb-0" style="color:black !important;font-size:16px">Thank you for choosing the installment payment option. The total amount will be divided into two installments. Please ensure that the next payment is made on the due date.</p>
                                        <p class="mb-0" style="color:rgb(231, 8, 8) !important;font-size:16px">Please note that opting for installment payments will incur an additional 5% fee.</p>
                                        </span>
								    </div> 
                            </div>
                            <div class="col-md-6">
							...
							@endif
							           ...
							           <tr>
                                        ...
                                        <tr>
                                            <td>Total Fee</td>
                                            <td>£ {{ round($mainprice + $mockprice - $data->discount_amount + $ilstallment_fees + $data->admin_specialaccess_amount + $data->special_access_initial_fees )}}</td>
                                        </tr>
                                        
                                    </tbody>
                                </table>
                                @php
                                    $price=$mainprice + $mockprice - $data->discount_amount + $ilstallment_fees + $data->admin_specialaccess_amount + $data->special_access_initial_fees ;
                        ...
                        <div class="col-md-6">
                           ...
                           <div class="form-group" id="card_new_payment_ils">
                                ...
                                <div class="form-group select-amount col-lg-12 col-md-12 col-sm-12 clearfix">
                                    ...
                                    <span style="color: #d24580; font-size:14px">Now you have to pay   £ {{ round($ilstrumentamount) }}</span>
                                </div>

                                <form action="{{ url('cardpayment-installment/session') }}" method="POST">
                                    @csrf
                                    
                                    <input type="hidden" name="amount" id="main_amount" value="{{ round($ilstrumentamount) }}">
                                    <input type="hidden" name="myamount" id="myamount" value="{{ $mainprice + $mockprice }}">
                                    <input type="hidden" name="cu_booking_id" id="cu_booking_id" value="{{ $data->booking_id }}">
                                    <input type="hidden" name="booking_id" value="{{ $data->booking_id }}">
                                    <input type="hidden" name="main_id" id="cu_main_id" value="{{ $data->id }}">
                                    <input type="hidden" name="user_id" id="cu_user_id" value="{{ $data->user_id }}">
                                    <input type="hidden" name="Installment_fee" value="{{ round($ilstallment_fees) }}">
                                    <button class="theme-btn btn-style-five" type="submit" id="checkout-live-button">Click here to pay with card ( £ {{ round($ilstrumentamount) }} )</button>
                                </form>
							...
							<div class="form-group" id="bank_ilstallmanet" style="display: none">
	                            <form method="post" action="{{ url('/bankpayment-installment/session') }}" enctype="multipart/form-data">
										@csrf
										<div class="row clearfix">
											...
											<input type="hidden" name="first_installment_date" value="{{ Carbon\Carbon::now()->format('d-m-Y') }}">
											<input type="hidden" name="second_installment_date" value="15-01-2024">
											<input type="hidden" name="total_amount" value="{{ $price }}">
											<input type="hidden" name="installment_amount" value="{{ round($ilstrumentamount) }}">
						                    <input type="hidden" name="booking_id" value="{{ $data->booking_id }}">
						                    <input type="hidden" name="examid" value="{{ $data->id }}">
                                            <input type="hidden" name="Installment_fee" value="{{ $ilstallment_fees }}">
										</div>
	                                <div class="form-group">
